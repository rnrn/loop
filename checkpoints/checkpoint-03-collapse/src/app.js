...
function renderCard(recording) {
  ...
  if (recording.transcriptStatus === 'ready') {
    ...
    // Change request: сворачиваем только блок текста.
    transcript.dataset.collapsible = 'true';
    transcript.setAttribute('aria-expanded', String(recording.expanded));
    transcript.tabIndex = 0;
    transcript.addEventListener('click', () => toggleCard(recording.id));

    const toggle = document.createElement('button');
    toggle.type = 'button';
    toggle.className = 'card__toggle';
    toggle.setAttribute('aria-expanded', String(recording.expanded));
    toggle.textContent = recording.expanded ? 'Свернуть' : 'Показать полностью';
    toggle.addEventListener('click', () => toggleCard(recording.id));

    li.append(meta, audio, transcript, toggle);
    return li;
  }
...
function toggleCard(id) {
  // Клик по audio сюда не попадает: обработчики висят только на тексте и кнопке.
  recordings = toggleExpanded(recordings, id);
  renderList();
}
